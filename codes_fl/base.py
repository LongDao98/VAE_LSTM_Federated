...
class BaseModel:
    ...
    def init_cur_epoch(self):
        with tf.variable_scope('cur_epoch_{}'.format(self.name)):
    ...
    def define_loss(self):
        with tf.name_scope("loss_{}".format(self.name)):
            ...
            with tf.Session() as sess:
                print ("elbo loss shape", sess.run(tf.shape(self.elbo_loss)))

    def training_variables(self):
        encoder_vars = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, "encoder_{}".format(self.name))
        decoder_vars = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, "decoder_{}".format(self.name))
        sigma_vars = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, "sigma2_dataset_{}".format(self.name))
...
class BaseTrain:
    ...
    def train(self):
        ...
        #for cur_epoch in range(0, self.config['num_comm_rounds'], 1):
        self.train_epoch()
