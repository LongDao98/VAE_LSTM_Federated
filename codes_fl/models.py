...
class VAEmodel(BaseModel):
  ...
  def build_model(self):
    ...
    with tf.variable_scope('encoder_{}'.format(self.name)):
      ...
      conv_1 = tf.layers.dense(inputs=input_tensor,
      ...
      encoded_signal = tf.layers.flatten(conv_1)
    ...
    with tf.variable_scope('decoder_{}'.format(self.name)):
      ...
      if self.config['l_win'] == 24:
        decoded_2 = tf.layers.dense(decoded_1,
                                  ...
                                  activation=tf.nn.leaky_relu,
                                  kernel_initializer=init)
        decoded = tf.layers.dense(decoded_2,
...
class lstmKerasModel:
  ...
  def produce_embeddings(self, model_vae, data, sess):
    ...
    for i in range(data.n_train_lstm):
      feed_dict = {model_vae.original_signal: np.reshape(data.train_set_lstm['data'][i],(-1,config['l_win'],config['n_channel'])),
